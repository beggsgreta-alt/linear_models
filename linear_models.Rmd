...
options(
  ...
  gglot2.continuous.fill = "viridis"
)
...
nyc_airbnb = 
  ...
  mutate(
    ...
    room_type = fct)infreq(room_type)
  )
```
...
what about the significance of `bourough`
...
```{r}
fit = lm(price ~ stars * bourough + room_type * borough, data = nyc_airbnb)
...
nyc_airbnb %>% 
  ...
  select(-data, -models) %>% 
  unnest(results) %>% 
  filter(str_detec(term, "room_type")) %>% 
